...
CONFIG = {
    ...
    "courses": ["Computer Science", "Data Structures", "Algorithms", "Machine Learning", "Web Development", "Database Systems"],
...
def load_student_data():
    ...
    for course in CONFIG["courses"]:
        ...
        for entry in course_data:
            entry["classes_attended"] = int((entry["attendance_percentage"] / 100) * entry["total_classes"])
        
        course_df = pd.DataFrame(course_data)
        course_file = os.path.join(CONFIG["data_dir"], f"{course.replace(' ', '_').lower()}_attendance.xlsx")
        ...
        for student in students:
            if 'attendance' in student and isinstance(student['attendance'], str):
                try:
                    ...
                    student['attendance'] = {course: 75.0 for course in CONFIG["courses"]}
...
def get_student_by_email(email):
    ...
    for student in students:
        if 'email' in student and student['email'] and student['email'].lower() == email:
            return student
...
def process_student_message(roll_no, subject, body):
    ...
    add_interaction(roll_no, f"SUBJECT: {subject}\nBODY: {body}", is_from_student=True)
    
    # Analyze message content
    response = analyze_message_content(roll_no, subject, body)
...
def analyze_message_content(roll_no, subject, body):
    ...
    if any(word in combined_text for word in ["hospital", "sick", "ill", "doctor", "medical", "health", "injury", "injured"]):
        # Try to extract duration if any
        recovery_days = extract_leave_duration(combined_text)
        
        if recovery_days:
            follow_up_date = (datetime.now() + timedelta(days=recovery_days)).strftime('%Y-%m-%d')
        else:
    ...
    for student in students:
        ...
        for follow_up_date in matches:
            if follow_up_date == today:
                ...
                circumstance_pattern = re.compile(r'TYPE: (.*?)\nDETAILS: (.*?)\nFOLLOW-UP DATE: ' + follow_up_date, re.DOTALL)
                ...
                if circumstance_match:
                    ...
                    follow_ups.append({
                        ...
                        "details": details,
                        "follow_up_date": follow_up_date,
                        "suggested_message": generate_follow_up_message(circumstance_type, student)
                    })
...
@app.route('/messages')
def messages():
    """Show messages from students that need responses"""
...
def follow_ups():
    """Show follow-ups that need to be done today"""
    follow_ups = check_follow_ups()
    
    return render_template('follow_ups.html', follow_ups=follow_ups)
